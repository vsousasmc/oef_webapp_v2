...
st.set_page_config(page_icon="🤖", page_title="Gerar Recomendação", layout="wide")
_max_width_()
...
with c30:
    ...
    st.number_input('Orçamento:', value=8400, step=1000,disabled =False)
    st.number_input('Custo de Impressão:', value=0.08, step=0.01,disabled =False)
    st.number_input('Número máximo de freguesias(modelo):', value=550, step=10,disabled =False)
    agree = st.checkbox('Excluir freguesias acima da distância minina definida?',disabled =False)
    st.text_input('Freguesias a excluir: ', placeholder="'000000', '000001', etc",disabled =False)
    st.write("##")
